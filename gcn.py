...
dataset_name='DGraph'
resume_path = f'/model-{gcn_parameters["num_layers"]}layers-{gcn_parameters["hidden_channels"]}hidden-{gcn_parameters["resume_epoch"]}epochs.pt'
final_path = f'/model-{gcn_parameters["num_layers"]}layers-{gcn_parameters["hidden_channels"]}hidden-{epochs}epochs.pt'
...
class GCN(torch.nn.Module):
    def __init__(self, feature, hidden, classes, layers=2) -> None:
        super().__init__()
        self.gcn1 = GraphConv(feature, hidden)
        self.gcn_middle = GraphConv(hidden, hidden)
        self.gcn2 = GraphConv(hidden, classes)
    ...
    def reset_parameters(self):
        ...
        self.gcn_middle.reset_parameters()
...
def init_model(data, nlabels=2, if_train=True):
    ...
    model = GCN(data.x.size(-1), hidden=gcn_parameters['hidden_channels'], classes=nlabels, layers=gcn_parameters['num_layers']).cuda()
    if if_train:
...
def run():
    # train_all(gcn_parameters['resume_epoch'])
    inference()
    # save_cpu_model()
    # score = get_score()
    # print(score)
