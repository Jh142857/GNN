...
dataset_name='DGraph'
resume_path = f'/model-{sage_parameters["num_layers"]}layers-{sage_parameters["hidden_channels"]}hidden-{sage_parameters["resume_epoch"]}epochs.pt'
final_path = f'/model-{sage_parameters["num_layers"]}layers-{sage_parameters["hidden_channels"]}hidden-{epochs}epochs.pt'
...
class GraphSAGE(torch.nn.Module):
    def __init__(self, feature, hidden, classes, layers=2) -> None:
        ...
        self.sage_middle = SAGEConv(hidden, hidden)
    ...
    def reset_parameters(self):
        ...
        self.sage_middle.reset_parameters()
...
def init_model(data, nlabels=2, if_train=True):
    ...
    model = GraphSAGE(data.x.size(-1), hidden=sage_parameters['hidden_channels'], classes=nlabels, layers=sage_parameters['num_layers']).cuda()
    if if_train:
...
def inference():
    data, split_idx, train_idx, nlabels = precess_data()
    model = init_model(data, nlabels=nlabels, if_train=True)
...
def summary_model():
    data, _, _, nlabels = precess_data()
    model = init_model(data, nlabels, if_train=False)
    summary(model, input_size=[data.x.shape])


def run():
    train_all(sage_parameters['resume_epoch'])
    # summary_model()
    # inference()
    # save_cpu_model()
    # score = get_score()
